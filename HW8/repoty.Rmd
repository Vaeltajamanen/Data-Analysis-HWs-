...
<p dir="RTL">
5. </br>
با توجه به نمودار به دست آمده، داستان در فصول میانی دارای فضایی ناراحت کننده بوده ولی در فصل آخر فضای داستان به سمت مثبت بودن میرود. 
</p>
```{r echo = F, include=T, message=FALSE, warning=F, error=FALSE}
...
<p dir="RTL">
ابتدا فصل های هر کتاب جداسازی شده اند و سپس به ازای هر فصل 
1-gram
و
2-gram
آن پیدا شده اند و توزیع کلمات محاسبه شده اند. برای نمونه نیز 6 نمودار از فصل های مختلف کتاب های مختلف رسم شده اند.
</p>
```{r echo = T, include=T, message=FALSE, warning=F, error=FALSE}
...
<p dir="RTL"> 
این بار از کتاب های جین آستن استفاده شده است و مانند بالا توزیع ها حساب شده اند. همچنین برای نمونه 6 نمودار کشیده شده است.3 نمودار اول مربوط به سه فصل مختلف از کتاب های
Sense & Sensibility
و
Pride & Prejudice
و
Emma 
 میباشند که توزیع 
 1 -gram
 برای 30 کلمه برتر آن ها رسم شده است.
 3
 نمودار دوم مربوط به همون فصول اند که 
 2 - gram
 آن ها برای 30 کلمه برتر رسم شده اند.
برای پیدا کردن شباهت آثار دو نویسنده از 
chisq.test
استفاده شده است.
</p>
```{r echo = T, include=T, message=FALSE, warning=F, error=FALSE}
...
<p dir="RTL"> 
کلمات مهم کتاب های دو نویسنده چارلز دیکنز و جین آستین و توزیع آن ها برای هر فصل محاسبه شده اند و به عنوان داده
train
به مدل
logestic 
میدهیم.
داده تست را بر روی کتاب های 
Oliver Twist
و 
Pride & Prejudice
میسازیم و به مدل میدهیم.
سپس خطای مدل را حساب میکنیم.
برای این محسابه 
2 gram
...
```
<p dir="RTL"> 
همچنین در زیر با استفاده از 
آزمون خیدو
میزان شباهت آثار این دو نویسنده بر حسب
2
gram
محاسبه شده است:
</p>
