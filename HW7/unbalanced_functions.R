...
AccuracyCutoffInfo <- function( train, test, predict, actual )
{
  ...
  plot <- ggplot( accuracy_long, aes( cutoff, accuracy, group = data, color = data ) ) + 
    ...
    ggtitle( "Train/Test Accuracy for Different Cutoff" )+ theme_hc(bgcolor = "darkunica") +
...
ROCInfo <- function( data, predict, actual, cost.fp, cost.fn )
{
  ...
  roc_plot <- ggplot( roc_dt, aes( fpr, tpr ) ) + 
    ...
    geom_vline( xintercept = best_fpr, alpha = 0.8, linetype = "dashed", color = "steelblue4" )	+ theme_hc(bgcolor = "darkunica") +
    scale_fill_hc("darkunica")			
  ...
  cost_plot <- ggplot( cost_dt, aes( cutoff, cost ) ) +
    ...
    geom_vline( xintercept = best_cutoff, alpha = 0.8, linetype = "dashed", color = "steelblue4" )	+ theme_hc(bgcolor = "darkunica") +
    scale_fill_hc("darkunica")
  
  # the main title for the two arranged plot
  # arranged into a side by side plot
  sub_title <- paste("Cutoff =  AUC = \n", 
                        best_cutoff, auc )
